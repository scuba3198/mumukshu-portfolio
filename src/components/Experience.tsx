...
const Experience = ({ experiences }: ExperienceProps) => {
	return (
		<section
			className="bento-card bg-white md:col-span-2 md:row-span-2"
		...
		>
			<div className="flex items-center gap-3 mb-6 border-b border-stone-100 pb-4 sticky top-0 bg-white z-10">
				...
				<h3 id="work-heading" className="font-bold text-lg">
					Work Experience
				</h3>
			</div>
			...
			<div className="space-y-8 overflow-y-auto custom-scrollbar pr-2 h-full max-h-[600px]">
				{experiences.map((job) => (
					<article
						key={job.id}
						className="relative pl-4 border-l-2 border-stone-200"
					>
						<div
							className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full ${job.color} border-4 border-white`}
						></div>
